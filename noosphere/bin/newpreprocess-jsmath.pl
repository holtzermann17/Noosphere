...
my @unsupported = ("\\begin{pspicture}", "\\xymatrix", "\\mathbbm", "\\mathbbmss");
...
#		print "Storing math: [$mathcontent]\n";
		...
		my $j = 0;
		if ( $text =~ /begin{eqnarray/ || $text=~ /begin{equation/ || $text =~ /begin{align/ || $text =~ /begin{pspicture/ ) {
#			print "begin equation type environment\n";
#			print "[$text]\n";
			$mathcontent = $text;
			for ( $j = $i+5; $j < $#tokens+1; $j++ ) {
				$text = join( '', @tokens[$j..($j+4)] );
				if ( $text =~ /end{eqnarray.*}/ ||  $text =~ /end{equation.*}/ || $text =~ /end{align.*}/ || $text =~ /end{pspicture}/ ) {
