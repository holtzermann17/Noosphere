...
sub htmlescape {
	...
	$text =~ s/&(?!\w+;)/&amp;/g;	# only escape &'s not being used in entities
	$text =~ s/</\&lt\;/g;
	$text =~ s/>/\&gt\;/g;
...
sub getTitle {
	...
	my $sth = $dbh->prepare("select title from objects where uid = ?");
	$sth->execute($objectid);

	my $row = $sth->fetchrow_hashref();
	my $title = $row->{'title'};
	#TODO: make this a mathy title
	
	return mathTitle($title);
}
